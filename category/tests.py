...
class ListTests(TestCase):
    ...
    def test_list_url_resolves_view(self):
        ...
        self.assertEquals(view.func.view_class, CategoryList)

    def test_list_view_contains_navigation_links(self):
        category_detail_url = reverse('category:detail', kwargs={'slug': 'graphic-design'})
        response = self.client.get(category_detail_url)
        self.assertContains(response, category_detail_url)

    def test_list_view_contains_new_navigation_links(self):
        new_category_url = reverse('category:new')
        response = self.client.get(new_category_url)
        # self.assertEqual(self.response.status_code, 302)
        self.assertContains(response, 'href="{0}"'.format(new_category_url))
...
class DetailTests(TestCase):
    ...
    def test_detail_view_success_status_code(self):
        url = reverse('category:detail', kwargs={'slug': 'graphic-design'})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    def test_detail_view_not_found_status_code(self):
        url = reverse('category:detail', kwargs={'slug': 'nothing-to-show-here'})
        response = self.client.get(url)
