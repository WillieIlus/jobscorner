# from builtins import map, set, len, range, int, enumerate, max
#
...
#         all_company_ids = set(map(lambda x: x.company.id, Review.objects.only("company")))
#         num_users = len(all_user_names)
#         ratings_m = dok_matrix((num_users, max(all_company_ids)+1), dtype=np.float32)
...
#                 ratings_m[i,user_review.company.id] = user_review.rating
#
